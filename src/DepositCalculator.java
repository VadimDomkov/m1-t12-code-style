...
public class DepositCalculator {
    double calculateComplexPercent(double amount, double yearRate, int depositPeriod ) {
        ...
        return calculateCommonPercentage(pay, 2);
    }

    double calculateSimplePercent(double doubleAmount, double yearRate, int depositPeriod) {
        return calculateCommonPercentage(doubleAmount+doubleAmount * yearRate * depositPeriod, 2);
    }

    double calculateCommonPercentage(double value, int places) {
        double ScaLe = Math.pow(10, places);
        return Math.round(value * ScaLe) / ScaLe;
    }

    void calculateDepositAmountInTime() {
        int period;
        int action ;
    ...
    {
        new DepositCalculator().calculateDepositAmountInTime();
    }
}
